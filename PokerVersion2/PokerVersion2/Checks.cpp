...
// broke up this function so if we add more this is the function we need to adjust
int Checks::FindHighCard(vector<string>hand,vector<string>cpuhand)
{
	vector<int> testhand = highCardValues(hand);
	vector<int> cputesthand = highCardValues(cpuhand);
...
//part removed from findhighcard so we can just pass any hand into it to determine its value
vector<int> Checks::highCardValues(vector<string>passedHand)
{
	int val;
